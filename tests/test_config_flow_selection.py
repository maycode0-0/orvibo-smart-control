...
_package = sys.modules.setdefault("orvibo_smart_control", ModuleType("orvibo_smart_control"))
...
def _import_module(name: str):
    ...
    spec = importlib.util.spec_from_file_location(f"orvibo_smart_control.{name}", mod_path)
    assert spec is not None and spec.loader is not None
    mod = importlib.util.module_from_spec(spec)
    sys.modules[f"orvibo_smart_control.{name}"] = mod
