...
[![Version](https://img.shields.io/badge/version-0.1.0-2f6f4e.svg)](custom_components/orvibo_smart_control/manifest.json)
...
设备列表拉取完成后会按实际设备名称自动归类，并使用云端返回的原始类型作为兜底；同类
多台设备合并显示，只有一台的类型保持独立。每个分类默认全选，也可以关闭该分类的全选
开关，再展开下拉列表逐台勾选。首次配置和“继续添加或移除设备”使用相同规则。
